####################################################################
#                                                                  #
#       '' crawlerDocumentaryMovies.py Documentation ''            #
# crawlerDocumentaryMovies.py harvest documentary movies from IMDB #
#                                                                  #
####################################################################

###########################################
#                                         #
# crawlerDocumentaryMovies.py handy tools #
#                                         #
###########################################
...
#################################################################
#                                                               #
# crawlerDocumentaryMovies.py ScrapeIMDBDocumentaryMovies class #
#         Run the scrapping of IMDB documentary movies.         #
#                                                               # 
#################################################################


class ScrapeIMDBDocumentaryMovies:
    def __init__(self):
        self.headers = {
    ...
    def scrape_documentary_movies(self, url, output_folder):
        try:
            ...
            output_file = os.path.join(output_folder, 'documentary_movies.json')
...
# Usage example:
scraper = ScrapeIMDBDocumentaryMovies()
url = 'https://www.imdb.com/search/title/?title_type=feature&genres=documentary'
output_folder = 'Database/JSONS'
scraper.scrape_documentary_movies(url, output_folder)
