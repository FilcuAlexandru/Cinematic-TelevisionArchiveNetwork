#####################################################################
#                                                                   #
#        '' crawlerAdventureTVSeries.py Documentation. ''           #
# crawlerAdventureTVSeries.py harvest adventure tv-series from IMDB #
#                                                                   #
#####################################################################

###########################################
#                                         #
# crawlerAdventureTVSeries.py handy tools #
#                                         #
###########################################
...
###############################################################
#                                                             #
# crawlerAdventureTVSeries.py ScrapeIMDBAdventureMovies class #
#          Run the scrapping of IMDB action movies.           #
#                                                             #  
###############################################################

class ScrapeIMDBAdventureTVSeries:
    def __init__(self):
        self.headers = {
    ...
    def scrape_adventure_tvseries(self, url, output_folder):
        try:
            ...
            output_file = os.path.join(output_folder, 'adventure_tvseries.json')
...
# Usage example:
scraper = ScrapeIMDBAdventureTVSeries()
url = 'https://www.imdb.com/search/title/?title_type=tv_series,tv_miniseries&genres=adventure'
output_folder = 'Database/JSONS'
scraper.scrape_adventure_tvseries(url, output_folder)
