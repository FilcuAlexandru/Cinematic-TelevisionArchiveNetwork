##########################################################################
#                                                                        #
#        '' crawlerDocumentaryTVSeries.py Documentation. ''              #
# crawlerDocumentaryTVSeries.py harvest documentary tv-series from IMDB  #
#                                                                        #
##########################################################################

#############################################
#                                           #
# crawlerDocumentaryTVSeries.py handy tools #
#                                           #
#############################################
...
#####################################################################
#                                                                   #
# crawlerDocumentaryTVSeries.py ScrapeIMDBDocumentaryTVSeries class #
#       Run the scrapping of IMDB documentary tv-series.            #
#                                                                   #  
#####################################################################

class ScrapeIMDBDocumentaryTVSeries:
    def __init__(self):
        self.headers = {
    ...
    def scrape_documentary_tvseries(self, url, output_folder):
        try:
            ...
            output_file = os.path.join(output_folder, 'documentary_tvseries.json')
...
# Usage example:
scraper = ScrapeIMDBDocumentaryTVSeries()
url = 'https://www.imdb.com/search/title/?title_type=tv_series,tv_miniseries&genres=documentary'
output_folder = 'Database/JSONS'
scraper.scrape_documentary_tvseries(url, output_folder)
